...
export default function SemesterCard({
  ...
  return (
    <div
      className={`semester-card ${overloaded ? 'semester-card--overloaded' : ''}`}
    >
      <div className="semester-card-header">
        <h3 className="semester-card-title">{semester.label}</h3>
        <span
          className={`semester-card-credits ${overloaded ? 'credits--warning' : ''}`}
        >
          {credits} credits
          {overloaded && (
            <span
              className="credits-warning-icon"
              title={`Exceeds ${MAX_CREDITS_PER_SEMESTER} credit limit`}
            >
              {' '}
              ⚠
            </span>
      ...
      <button
        ...
        onClick={() => onAddClick(semester.id)}
      >
        + Add Course
      </button>
    </div>
